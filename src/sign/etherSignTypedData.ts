...
export const etherSignTypedData = async (
  ...
  if (wallet === Wallet.METAMASK) {
    return await provider.send("eth_signTypedData_v4", [address, JSON.stringify(rpcData)]); // MetaMask
  }
  return await provider.send("eth_signTypedData", [address, JSON.stringify(rpcData)]); 
};
